...
export default function ProjectDetails() {
    ...
    const fetchProject = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log(res)
          setProject(res?.data?.project ?? res?.data ?? null);
          return;
        } catch (err) {
          // ignore and fall back
        }

        // Fallback: fetch all projects and find by id
        const listRes = await api.get('/api/client/myprojects');
        const list = listRes?.data?.projects ?? listRes?.data ?? [];
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F8F8] flex items-center justify-center md:ml-0 px-6">
